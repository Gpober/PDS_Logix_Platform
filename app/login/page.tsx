...
import { PdsMark } from '@/components/PdsMark';
...
function LoginForm() {
  ...
  async function handleGoogle() {
    ...
    // where they were headed after the code exchange. Identity scopes only.
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-sm">
      <div className="text-center">
        <Link href="/" className="inline-flex items-center gap-2 font-display text-2xl font-semibold">
          <PdsMark className="h-7 w-7 text-pds" />
          PDS Logix
        </Link>
      ...
      <button
        ...
        className="w-full rounded-full bg-ink px-6 py-3 text-white transition-colors hover:bg-steel disabled:opacity-60"
      >

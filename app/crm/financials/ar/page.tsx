import { getArAging } from '@/lib/integrations/pdsbooks';
...
export default async function ArPage() {
  ...
  if (res.status === 'error') return <Empty>Couldn’t read receivables: {res.message}</Empty>;

  const { rows, totals } = res.data;

  return (
    <div className="space-y-6">
      <p className="text-center text-sm text-stone">Accounts receivable — who owes us, by age (as of today)</p>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        <Kpi label="Total open" value={usd(totals.total)} />
      ...
      {rows.length === 0 ? <Empty>No open receivables.</Empty> : (
        <div className="overflow-x-auto rounded-2xl border border-line bg-white">
          <table className="w-full text-sm">
            <thead className="border-b border-line text-left text-xs uppercase tracking-wider text-stone">
              <tr><th className="px-4 py-3 font-medium">Client</th><th className="px-4 py-3 font-medium">Current</th><th className="px-4 py-3 font-medium">31–60</th><th className="px-4 py-3 font-medium">61–90</th><th className="px-4 py-3 font-medium">90+</th><th className="px-4 py-3 font-medium">Total</th></tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.customer} className="border-t border-line hover:bg-blush/30">
                  <td className="px-4 py-2.5 text-ink">{r.customer}</td>
